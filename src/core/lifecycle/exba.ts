...
import { batch, computed, type Signal, signal } from '@core/reactivity';
...
export class EXBA {
  ...
  static DEBUG = true;
  /** Whether to strictly validate component blueprints during registration */
  static VALIDATE_BLUEPRINTS = true;
   ...
   * This is used by both TS and WASM to drive the reactive UI.
   * @param key The unique key of the signal
  ...
  /**
   * Batch multiple signal updates into a single microtask to prevent redundant re-renders.
  ...
  /**
   * Entry point for WASM to push a state update to the frontend.
   * Usually called by the Rust bridge when an internal state change occurs.
   * @param patch A map of signal keys and their new values
   */
  static pushState(patch: Record<string, any>) {
    EXBA.batch(() => {
  ...
  // ─── WASM ───────────────────────────────────────────────────
  /**
   * Internal buffer for pending WASM state updates to prevent "chatty" bridge communication.
   */
  private static _stateBuffer = new Map<string, any>();
  private static _syncTimeout: any = null;

  /**
   * Binds a local or global signal to a WASM state key with buffered synchronization.
   */
  static bindSignalToWasm<T>(signal: Signal<T>, wasmKey: string) {
    const originalSetter = signal[1];
    
    signal[1] = (val: T | ((prev: T) => T)) => {
      const nextVal = typeof val === 'function' ? (val as Function)(signal[0]()) : val;
      originalSetter(nextVal);
      
      // Buffer the update instead of calling WASM immediately
      this._stateBuffer.set(wasmKey, nextVal);
      this.scheduleSync();
    };
  }
  ...
  private static scheduleSync() {
    ...
    }, 16); // Sync once per frame (~60fps)
  }

  /**
   * Flushes the state buffer and sends a single bulk update to WASM.
   */
  static commitState() {
    if (this._stateBuffer.size === 0) return;

    const patch = Object.fromEntries(this._stateBuffer);
    this._stateBuffer.clear();

    if (this.bridge) {
      const payload = JSON.stringify(patch);
      const wasmUpdate = (window as any).wasm_update_app_state || this.wasmModule?.wasm_update_app_state;
  ...
  /**
   * Initializes the WebAssembly module and all registered Domain Services.
   */
  static async initWasm(initFn: any) {
    ...
    try {
      ...
      // Meta-Feature: Auto-initialize all registered domain services
      const { Domains } = await import('@core/wasm/registry');
      await Domains.initAll();
  ...
  /**
   * Middleware type for intercepting bridge calls.
   */
  static middleware: Array<(ctx: { method: string; args: any[]; next: () => Promise<any> }) => Promise<any>> = [];

  /**
   * Registers a middleware function to the bridge pipeline.
   */
  static useMiddleware(fn: (ctx: { method: string; args: any[]; next: () => Promise<any> }) => Promise<any>) {
    this.middleware.push(fn);
  }

  /**
   * Manual entry point for calling bridge methods with type safety.
   * Now supports a middleware pipeline.
   */
  static async callBridge<T>(method: string, ...args: any[]): Promise<T> {
    ...
    const execute = async (index: number): Promise<T> => {
      if (index < EXBA.middleware.length) {
        return EXBA.middleware[index]({
          method,
          args,
   ...
   * @returns A signal with tuple API, value getter/setter and subscribe method
   */
  static createSignal<T>(initialValue: T, key?: string): Signal<T> {
   ...
   * @returns A memo containing the computed value
   */
  static createComputed<T>(fn: () => T, dependencies: string[]): any {
