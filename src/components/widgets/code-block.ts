...
export class CodeBlockComponent extends ExbaComponent {
  ...
  private highlight(code: string): string {
    ...
    escaped = escaped.replace(
      /\b(export|import|const|let|var|function|return|if|else|for|while|class|interface|type|enum|struct|pub|fn|use|mod|impl|match|move|async|await)\b/g,
      '<span class="keyword">$1</span>',
    );

    // 4. Highlight types
    escaped = escaped.replace(
      /\b(string|number|boolean|any|void|Self|String|i32|f64|Vec|Option|Result|Mutex|u32)\b/g,
