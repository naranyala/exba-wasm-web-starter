...
export class LeafletTurfComponent extends ExbaComponent {
  ...
  static styles = {
    container:
      'padding: 2rem; width: 100%; max-width: 1200px; margin: 0 auto; display: flex; flex-direction: column; gap: 1.5rem;',
  ...
  private calculateConvexHull() {
    ...
    if (points.length < 3) {
      this.setState({
        analysisText: 'Need at least 3 points for a Convex Hull!',
      });
      return;
    }
    ...
    if (hull) {
      ...
      this.setState({
        analysisText: 'Convex Hull and Center of Mass calculated',
      });
    }
  }
