...
export class VegaLiteComponent extends ExbaComponent {
  ...
  static styles = {
    container:
      'padding: 2rem; width: 100%; max-width: 1000px; margin: 0 auto; display: flex; flex-direction: column; gap: 1.5rem;',
    ...
    controls:
      'display: flex; gap: 0.75rem; margin-bottom: 1rem; flex-wrap: wrap; justify-content: center;',
    ...
    if (type === 'bar') {
      ...
      spec = {
        ...
        encoding: {
          ...
          tooltip: [
            { field: 'symbol' },
            { field: 'date', timeUnit: 'yearmonthdate' },
            { field: 'price' },
          ],
        },
        config: darkConfig,
        ...
        marks: [
          ...
          },
          {
            ...
            encode: {
              enter: {
                x: {
                  signal: "(radius + 15) * cos(scale('angular', datum.key))",
                },
                y: {
                  signal: "(radius + 15) * sin(scale('angular', datum.key))",
                },
                text: { field: 'key' },
                align: [
                  {
                    test: "abs(scale('angular', datum.key)) > PI / 2 + 0.1",
                    value: 'right',
                  },
                  {
                    test: "abs(scale('angular', datum.key)) < PI / 2 - 0.1",
