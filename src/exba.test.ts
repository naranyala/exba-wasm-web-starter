...
describe('EXBA IR System', () => {
  ...
  describe('IRProcessor LLIR Execution', () => {
    it('should update text correctly', async () => {
      ...
      expect(document.getElementById('test-el')?.textContent).toBe(
        'Updated Text',
      );
    });
    ...
    it('should set attributes correctly', async () => {
      ...
      await new Promise((r) => setTimeout(r, 100));
      expect(
        document.getElementById('test-el')?.getAttribute('data-test'),
      ).toBe('value');
    });
    ...
    it('should trigger custom events', async () => {
      ...
      const spy = vi.fn();
      eventEl.addEventListener('exba-event', spy);

      const bundle: IRBundle = {
        version: '1.0.0',
        hlir: null,
        llir: [{ type: 'TriggerEvent', id: 'event-el', event: 'exba-event' }],
      };
      IRProcessor.process(bundle);
      await new Promise((r) => setTimeout(r, 100));
    ...
    it('should handle Log instructions without crashing', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('[EXBA-LOG] Test log'),
        expect.any(String),
      );
    ...
    it('should handle Anomaly instructions with high priority log', () => {
      ...
      const bundle: IRBundle = {
        ...
        hlir: null,
        llir: [
          { type: 'Anomaly', code: 'ERR_001', details: 'Something went wrong' },
        ],
      };
      IRProcessor.process(bundle);
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('[EXBA-ANOMALY] ERR_001'),
        expect.any(String),
      );
      consoleSpy.mockRestore();
    });
  ...
  describe('Edge Cases & Corners', () => {
    ...
    it('should process HLIR for analysis', () => {
      const exbaSpy = vi.spyOn(EXBA, 'log');
      const bundle: IRBundle = {
        version: '1.0.0',
      ...
      IRProcessor.process(bundle);
      expect(exbaSpy).toHaveBeenCalledWith('IR_HLIR_ANALYSIS', bundle.hlir);
      exbaSpy.mockRestore();
    });

    it('should maintain atomic order of execution', async () => {
      const bundle: IRBundle = {
        version: '1.0.0',
        hlir: null,
        llir: [
      ...
      IRProcessor.process(bundle);
      await new Promise((r) => setTimeout(r, 100));
      expect(document.getElementById('test-el')?.textContent).toBe('Third');
    });
  });

  describe('Bridge Integration', () => {
    it('should correctly dispatch IR via EXBA.dispatchIR', async () => {
      const bundle: IRBundle = {
        version: '1.0.0',
        hlir: null,
        llir: [{ type: 'UpdateText', id: 'test-el', text: 'Bridge Test' }],
      };
      EXBA.dispatchIR(bundle);
      await new Promise((r) => setTimeout(r, 100));
      expect(document.getElementById('test-el')?.textContent).toBe(
        'Bridge Test',
      );
