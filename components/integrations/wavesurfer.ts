...
export class WavesurferComponent extends ExbaComponent {
  ...
  static styles = {
    ...
    controls: 'display: flex; gap: 1rem; align-items: center; justify-content: center; margin-top: 1rem;',
  ...
  protected onMount() {
    ...
    const fileInput = this.shadowRoot?.getElementById('audio-upload') as HTMLInputElement;
    const fileNameDisplay = this.shadowRoot?.getElementById('file-name');
    const visSelect = this.shadowRoot?.getElementById('vis-select') as HTMLSelectElement;
    ...
    if (visSelect) {
        visSelect.addEventListener('change', (e) => {
            this.visualizationType = (e.target as HTMLSelectElement).value as 'bar' | 'smooth' | 'solid';
            this.initWavesurfer();
        });
    }
  ...
  private initWavesurfer() {
    ...
    if (this.currentObjectUrl) {
        ...
        this.wavesurfer.load('https://wavesurfer.xyz/wavesurfer-code/examples/audio/audio.wav');
    }
  }
