...
export class TSParser extends BaseParser {
  ...
  parseFile(filePath: string): ParserResult {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const ignoredKeywords = ['if', 'for', 'while', 'switch', 'catch', 'return', 'await', 'yield', 'super'];
      if (ignoredKeywords.includes(name)) continue;
      ...
      // If it's a method inside a class, it might not have 'export'
      // But we should ensure it's actually a method declaration
      const isMethod = !!currentContainer && trimmed.match(/^[a-zA-Z0-9_]+\s*\(/);

      if (!isExported && !isMethod) continue;

      let type: DocEntry['type'] = 'variable';
      if (trimmed.match(/^export\s+const/)) type = 'constant';
      else if (trimmed.match(/^export\s+(?:let|var)/)) type = 'variable';
      else if (trimmed.includes('function')) type = 'function';
      ...
      else if (trimmed.match(/^(?:export\s+)?([a-zA-Z0-9_]+)\s*\(/)) {
